...
from .constants import PLAYER_HUMAN_ID  # PLAYER_AI_ID burada kullanılmıyor ama tutarlılık için kalabilir
...
class Unit:
    ...
    def __init__(self, grid_x, grid_y, unit_type, player_id, color=(0, 0, 255), size=30):
        ...
        self.has_acted_this_turn = False  # !!! YENİ BAYRAK !!!
    ...
    def draw(self, surface):
        ...
        if self.rect:
            ...
            # Eğer birim bu tur eylem yaptıysa rengini biraz soluklaştırabiliriz (isteğe bağlı)
            if self.has_acted_this_turn:
                # Soluk renk: (R,G,B) -> (R*0.6, G*0.6, B*0.6) gibi
                dim_factor = 0.6
                current_color = (int(self.color[0] * dim_factor),
                                 int(self.color[1] * dim_factor),
                                 int(self.color[2] * dim_factor))
            ...
            if self.health < self.max_health:
                ...
                health_bar_rect = pygame.Rect(self.pixel_x, self.pixel_y - bar_y_offset, bar_width, bar_height)
                pygame.draw.rect(surface, (150, 0, 0), background_bar_rect)  # Daha koyu kırmızı
                pygame.draw.rect(surface, (0, 200, 0), health_bar_rect)  # Biraz daha parlak yeşil
    ...
    def get_tiles_in_movement_range(self, game_map):
        in_range_tiles = []
        if not self.is_alive() or self.has_acted_this_turn: return in_range_tiles  # Eylem yaptıysa hareket edemez
    ...
    def get_tiles_in_attack_range(self, game_map):
        ...
        if not self.is_alive() or self.has_acted_this_turn: return in_range_attack_tiles  # Eylem yaptıysa saldıramaz
    ...
    def die(self):
        print(f"Player {self.player_id}'s {self.unit_type} (ID:{self.id}) at ({self.grid_x}, {self.grid_y}) has died!")
    ...
    def to_dict(self):
        unit_data = {
            ...
            "has_acted_this_turn": self.has_acted_this_turn,  # !!! BAYRAĞI KAYDET !!!
        }
        return unit_data

    def __str__(self):
        return f"ID:{self.id} P{self.player_id} {self.unit_type} ({self.health}HP) Acted:{self.has_acted_this_turn} at ({self.grid_x}, {self.grid_y}) in state {self.current_state_name}"
